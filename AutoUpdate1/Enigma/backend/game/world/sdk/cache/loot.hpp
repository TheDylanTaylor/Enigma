...
//{
//	return read <uintptr_t>(actor + offsets->rootcomp);
//}
//Vector3 GetLocationn(uintptr_t actor)
//{
//	return  read <Vector3>(RootComponentt(actor) + offsets->relativelocation);
//}
...
//		uint64_t NamePoolChunk = read<uint64_t>(globals->imagebase + FNAMEPOOL_OFFSET + (8 * ChunkOffset) + 16) + (unsigned int)(2 * NameOffset);
...
//		if (v4)
//		{
//			read_process((NamePoolChunk + 2), (PVOID)buff, 2 * nameLength);
//			do
//			{
//				v8 = v6++;
//				v9 = (v8 | 0xB000) + v7;
...
//				mrxd.reserve(100);
//				uintptr_t ItemLevels = read<uintptr_t>(cached->gworld + 0x170);
//				DWORD itemSize = read<DWORD>(cached->gworld + (0x170 + sizeof(PVOID)));
//
//				for (int i = 0; i < itemSize; ++i) {
//					uintptr_t ItemLevel = read<uintptr_t>(ItemLevels + (i * sizeof(uintptr_t)));
//
//					for (int j = 0; j < read<DWORD>(ItemLevel + (0xA0 + sizeof(PVOID))); ++j) {
...
//		//			{
//		//				auto definition = read <uint64_t>(entity.Actor + 0x350 + 0x18); // AFortPickup::PrimaryPickupItemEntry : FFortItemEntry::ItemDefinition -> 0x18
//		//				std::cout << "def -> " << definition << "\n";
//		//				if (ioctl.is_valid(definition))
//		//				{
//		//					BYTE tier = read <BYTE>(definition + 0x11b); //UFortItemDefinition::Tier
//		//					ImColor Color, RGBAColor;
//		//					Vector3 Location = GetLocationn(entity.Actor);
//		//					Vector2 ChestPosition = uengine->w2s(Location);
//		//					uint64_t ftext_ptr = read <uint64_t>(definition + 0x30);
...
//		//							read_process(ftext_data, ftext_buf, ftext_length * sizeof(wchar_t));
