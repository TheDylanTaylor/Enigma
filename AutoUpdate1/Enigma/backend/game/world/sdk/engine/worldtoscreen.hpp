...
struct unrealengine
{
	...
	__forceinline auto bonewithrotation(uintptr_t MESH, int ID) -> Vector3
	{
		...
		uint64_t boneArray = read<uint64_t>(MESH + offsets->bonearray + (read<int>(MESH + offsets->bonecache) * 0x10));
		//printf("boneArray -> %llx\n", (uintptr_t)boneArray);
		FTransform bone = read<FTransform>(boneArray + (ID * offsets->bonec));

		FTransform componentToWorld = read<FTransform>(MESH + offsets->comptowrld);
	...
	__forceinline auto viewpoint(uintptr_t gworld, uintptr_t playercontroller) -> cdecrypt
	{
		...
		auto locationp = read<uintptr_t>(gworld + 0x110);
		auto rotationp = read<uintptr_t>(gworld + 0x120);
